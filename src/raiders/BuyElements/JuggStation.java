...
public class JuggStation extends BuyStation{
    
    private final Image jugg = new ImageIcon("/Users/21sleshko/NetBeansProjects/Raiders/JugBuy.png").getImage();

    private Perks perks;
    private boolean showMessage = false;
    
    public JuggStation(float x, float y, Player player, int price, String name, Perks perks) {
    ...
    public void draw(Graphics g){
        ...
        g.drawImage(jugg, (int) x, (int) y, null);
        if(showMessage && !perks.hasJug){
    ...
    @Override
    public void buy(){
        if(showMessage && player.getPoints() >= price && !perks.hasJug){
