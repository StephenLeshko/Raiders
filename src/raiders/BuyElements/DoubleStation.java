...
public class DoubleStation extends BuyStation{
    
    private final Image doubleTap = new ImageIcon("/Users/21sleshko/NetBeansProjects/Raiders/DoubleTapBuy.png").getImage();

    private Perks perks;
    private boolean showMessage = false;
    
    public DoubleStation(float x, float y, Player player, int price, String name, Perks perks) {
    ...
    public void draw(Graphics g){
        ...
        g.drawImage(doubleTap, (int) x, (int) y, null);
        if(showMessage && !perks.hasDouble){
    ...
    @Override
    public void buy(){
        if(showMessage && player.getPoints() >= price && !perks.hasDouble){
